...
const createOrg = async (req, res) => {
  try {
    const { name, address, type,contact, industry, website, password } = req.body;

    // Check if all required fields are provided
    if (!name || !address || !contact || !industry || !website || !password) {
      return res.status(400).json({ message: "All fields are required with password" });
    }
